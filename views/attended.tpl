...
<div class="fpbx-container" style="margin-top:20px;">
        ...
        <form class="form-inline" role="form" name="Procurar" method='post'>
          ...
          <div class="btn-group pull-right btn-action">
              ...
              <?php //if($data_from_session): ?>
                <button type="submit" value="1" name="attended[limpar]" class="btn btn-default">
          ...
          <div class="form-group">
            ...
            <input type="text" name="attended[data_inicio]" value="<?php echo $data_inicio; ?>" class="input-sm form-control" style="width: 95px;font-weight:bolder !important;" placehold="Data inicial" />
            <input type="text" name="attended[hora_inicio]" value="<?php echo $hora_inicio; ?>" class="input-sm form-control" style="width: 60px;font-weight:bolder !important;" placehold="Hora inicial" />
          </div>

          <div class="form-group">
            <small><b><?php echo _('Until');?>:</b></small>
            <input type="text" name="attended[data_fim]" value="<?php echo $data_fim; ?>" class="input-sm form-control"  style="width: 95px;font-weight:bolder !important;" placehold="Data final"  />
            <input type="text" name="attended[hora_fim]" value="<?php echo $hora_fim; ?>" class="input-sm form-control" style="width: 60px;font-weight:bolder !important;" placehold="Hora final" />
          </div>
          <br />
        </form>
    </div>
  ...
  <h2><?php echo _('Total Attendant'); ?></h2>
  <div class="row" style="border:1px solid #d2d2d2;margin:10px 5px 10px 5px;text-align:center;">
    <div class="col-md-4">
        ...
        <h2><?php echo $results['general']['answered']; ?></h2>
    </div>
    <div class="col-md-2">
        <b><small> <?php echo _('Total Time'); ?> </small></b>
        <h2><?php echo gmdate('H:i:s', $results['general']['duration']); ?></h2>
    </div>
    <div class="col-md-2">
    ...
    <div class="col-md-2">
        ...
        <h2><?php echo gmdate('H:i:s', $results['general']['avg_answered']); ?></h2>
    </div>
    <div class="col-md-2">
        <b><small> <?php echo _('Average Hold Time'); ?></small></b>
        <h2><?php echo gmdate('H:i:s', $results['general']['avg_holdtime']); ?></h2>
    </div>
  </div>
  ...
  <?php foreach($results['attendant'] as $att => $v):  ?>
    ...
    <div class="row" style="border:1px solid #d2d2d2;margin:0 5px;text-align:center;">
      <div class="col-md-4">
          <b><small> <?php echo _('Answered'); ?> </small></b>
          <h2><?php echo $v['answered']; ?></h2>
      </div>
      <div class="col-md-2">
          <b><small> <?php echo _('Total Time'); ?> </small></b>
          <h2><?php echo gmdate('H:i:s', $v['duration']); ?></h2>
      </div>
      <div class="col-md-2">
          <b><small> <?php echo _('Total Hold Time'); ?></small></b>
          <h2><?php echo gmdate('H:i:s', $v['holdtime']); ?></h2>
      </div>
      <div class="col-md-2">
          <b><small> <?php echo _('Average Call Time'); ?></small></b>
          <h2><?php echo gmdate('H:i:s', $v['avg_answered']); ?></h2>
      </div>
      <div class="col-md-2">
          <b><small> <?php echo _('Average Hold Time'); ?></small></b>
          <h2><?php echo gmdate('H:i:s', $v['avg_holdtime']); ?></h2>
      </div>
    </div>
  <?php endforeach; ?>
  ...
  <table class="table table-striped table-hover table-responsive">
    ...
    <tbody>
        <?php if(count($results['analitic']) > 0): ?>
          <?php foreach($results['analitic'] as $evento) : ?>
              <tr>
...
<a href="/admin/config.php?display=callsreport&action=report&report=attended&pdf=1" class="btn btn-primary pull-right" target="_blank">
...
<script type="text/javascript">
  ...
  var splayer = false;
	...
	$(document).ready(function() {
        ...
        jQuery("input[name='attended[data_inicio]']").datetimepicker({
          timepicker:false,
          format:'d/m/Y',
          lang:'pt'
        });

        jQuery("input[name='attended[data_fim]']").datetimepicker({
          timepicker:false,
          format:'d/m/Y',
          lang:'pt'
        });

        jQuery("input[name='attended[hora_inicio]']").datetimepicker({
          datepicker:false,
          format:'H:i',
          lang:'pt'
        });

        jQuery("input[name='attended[hora_fim]']").datetimepicker({
          datepicker:false,
          format:'H:i',
          lang:'pt'
        });
